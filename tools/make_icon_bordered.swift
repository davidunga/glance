#!/usr/bin/env swift
// Variant of make_icon_clean.swift with a thin border drawn in the same
// grey-blue as the three text bars. Writes a preview PNG plus a full
// .iconset so the result can be compared before promotion.
//
// Usage: swift tools/make_icon_bordered.swift

import AppKit
import Foundation

let outDir = URL(fileURLWithPath: "AppIcon-bordered.iconset")
let previewURL = URL(fileURLWithPath: "AppIcon-bordered-preview.png")
...
func render(size px: Int) -> Data {
    ...
    NSBezierPath(roundedRect: fullRect, xRadius: radius, yRadius: radius).addClip()

    // Paper gradient — same as clean variant.
    ...
    // Three text bars — identical geometry to the clean variant.
    ...
    // Thin border in the text-bar color. Drawn unclipped so the stroke sits
    // flush with the outer rounded edge (not half-eaten by the clip).
    let strokeWidth = max(1, s * 0.012)
    let inset = strokeWidth / 2
    let strokePath = NSBezierPath(
        roundedRect: fullRect.insetBy(dx: inset, dy: inset),
